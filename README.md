...
사용자가 모호한 결정 지점에서 **1/2/3 + Other 텔레그램 인라인 키보드**로 응답할 수 있다. 4 앵커 중 1-3 자동 트리거 (planner 다중해석 / GAP_DETECTED / vague exec). prompts/reviewer.md 본문 무변경 (우로보로스 정합).

```bash
# 직접 ask (구조화 선택지)
skills/ohmyclaw/cli.sh ask --to $CHAT_ID \
  --question "어떤 DB?" --option 1:Postgres --option 2:Mongo --option 3:Redis --other

# Anchor 3: 모호한 task 자동 ask
skills/ohmyclaw/cli.sh exec "이거 해줘" --to $CHAT_ID
# → ambiguity score>0.2 감지 → 3 해석 옵션 + Other 자동 발동

# Anchor 1: planner 출력 → ask
skills/ohmyclaw/cli.sh plan "API 마이그레이션" | skills/ohmyclaw/cli.sh plan-gate --to $CHAT_ID

# Anchor 2: GAP_DETECTED → 사용자 결정 (apply-fix / ignore-gap / Other)
echo "$REVIEWER_JSON" | skills/ohmyclaw/cli.sh gap-gate --to $CHAT_ID
```

Ouroboros 4차원 가중 Ambiguity Score(`goal 0.35 + constraint 0.25 + success 0.25 + context 0.15`, threshold 0.2) 로 게이팅. 응답은 state.sh 에 자동 저장되고 후속 verb 진입 시 `OHMYCLAW_LAST_ANSWER` env 로 prefetch.

자세한 설계·다이어그램·테스트 커버리지: [skills/ohmyclaw/docs/ask-flow.md](skills/ohmyclaw/docs/ask-flow.md)
