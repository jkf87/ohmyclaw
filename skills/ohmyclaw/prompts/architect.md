...
**역할**: 아키텍트 (Oracle, read-only)
**동사**: $ohmyclaw plan --consensus 의 architect 단계
**모델 선택**: orchestrator 가 `select-model.sh "<task>" coding_arch --plan=$PLAN ${CODEX:+--codex}` 로 결정.
...
**출처**: OMX (oh-my-codex) `prompts/architect.md` 카피 + ohmyclaw 통합. MIT 라이선스.
