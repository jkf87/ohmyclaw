...
### 비동기 상태머신 — 실제 버튼 인터랙션 (v1.8.0)

동기 `interview <topic>` 는 CLI 1회성이라 **버튼 클릭을 받을 수 없다**(openclaw 는 콜백을 비동기로 에이전트 턴에 전달 → 이미 끝난 프로세스엔 도달 못 함 → `degraded:true` 폴백). 실제 클릭 동작을 위해 **재개 가능 상태머신 + 에이전트 구동** 모드를 제공한다.

```
/ohmyclaw interview <topic>
        │  에이전트 → cli.sh interview start "<topic>" --to <chatId>
        ▼
[interview-session state 저장] → Q1 발화 (command 버튼: /omc_iv feature ...)
        │
        ▼  사용자 버튼 클릭
openclaw: command 버튼 → synthetic 메시지 "/omc_iv feature" → 에이전트 인바운드
        │  에이전트(SKILL.md) → cli.sh interview answer "feature"
        ▼
[answer 기록(fallback:false) → 재채점 → 다음 질문 or 종료]
        │  (모호성 ≤ threshold → 종료)
        ▼
interview-result 저장 (mode:"async", degraded:false) + 요약 발송
```

- **버튼 타입**: `action.type="command"`, `command:"/omc_iv <value>"`. openclaw 가 클릭을 synthetic 슬래시 명령으로 에이전트에 전달(`buildSyntheticTextMessage`+inbound). `callback` 타입과 달리 받는 주체(에이전트)가 항상 존재.
- **CLI 표면**: `interview start <topic> --to <chatId>` / `interview answer <value>` / `interview status` / `interview cancel`. `answer` 의 `--to` 는 세션에서 자동 로드.
- **Other**: `/omc_iv __other__` → 자유입력 안내(awaiting 유지) → 다음 텍스트를 `interview answer "<텍스트>"` 로.
- **정직성**: 클릭으로 받은 답은 실제 선택 → `degraded:false`, 답변별 `fallback:false`.
- 에이전트 오케스트레이션 규약: [SKILL.md §1 "인터뷰 비동기 오케스트레이션"].
- 테스트 모드: `OHMYCLAW_ASK_MOCK=1` → presentation 발화 dry-run(실 openclaw 미호출).
